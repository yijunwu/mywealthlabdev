namespace TASCIndicators
{
    ...
    public class VMMinus : DataSeries
    {
        private Bars _bars;
        private int _period;
        private DataSeries _tr;
        private DataSeries _vmMinus;

        public VMMinus(Bars bars, int period, string description) : base(bars, description)
        {
            this._bars = bars;
            this._period = period;
            ...
            this._vmMinus = Sum.Series(DataSeries.Abs(bars.Low - (bars.High >> 1)), period);
            for (int i = period; i < bars.Count; i++)
            {
                base[i] = this._vmMinus[i] / this._tr[i];
            }
        }
        ...
        public static VMMinus Series(Bars bars, int period)
        {
            DataSeries series;
            string key = "VM-(" + period + ")";
            if (bars.Cache.ContainsKey(key))
            {
                return (VMMinus) bars.Cache[key];
            }
            bars.Cache[key] = series = new VMMinus(bars, period, key);
            return (VMMinus) series;
        }
        ...
        public class VMPlusHelper : IndicatorHelper
        {
            ...
            public override string URL
            {
                get
                {
                    return "http://www2.wealth-lab.com/WL5Wiki/VMMinus.ashx";
                }
            }
        }
    }
