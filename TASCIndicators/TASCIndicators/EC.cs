namespace TASCIndicators
{
    ...
    public class EC : DataSeries
    {
        ...
        public static EC Series(DataSeries ds, int Length, int Gain)
        {
            string key = string.Concat(new object[] { "EC(", ds.Description, ",", Length, ",", Gain, ")" });
            if (ds.Cache.ContainsKey(key))
            {
                return (EC) ds.Cache[key];
            }
            EC ec = new EC(ds, Length, Gain, key);
            ds.Cache[key] = ec;
            return ec;
        }
    }
