namespace WealthLab.IndexDefinitions
{
    ...
    public class AboveSMA : IndexDefinition
    {
        private NumericParameter _paramUI = new NumericParameter("Period:", 8M, 0, 2147483647M, -2147483648M, 1M);
        ...
        public override void SetIndexValues(List<Bars> bars, List<int> barNumbers, DateTime dt, Bars indexToDate)
        {
            if (bars.Count != 0)
            {
                ...
                for (int i = 0; i < bars.Count; i++)
                {
                    ...
                    if (num4 >= SMA.Series(bars2.Close, this._period).FirstValidValue)
                    {
                        num++;
                        if (bars2.Close[num4] > SMA.Series(bars2.Close, this._period)[num4])
                        {
                            open++;
                        }
                    }
        ...
        public override string Description
        {
            get
            {
                return "An indicator that ranges from 0 to 100, and expresses the percentage of the symbols in the selected DataSet that are above a Simple Moving Average of a specific period.";
            }
        }
