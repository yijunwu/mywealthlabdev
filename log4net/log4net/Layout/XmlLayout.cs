namespace log4net.Layout
{
    ...
    public class XmlLayout : XmlLayoutBase
    {
        ...
        public XmlLayout()
        {
            ...
            this.m_base64Message = false;
            this.m_base64Properties = false;
        }
        ...
        protected override void FormatXml(XmlWriter writer, LoggingEvent loggingEvent)
        {
            ...
            if (properties.Count > 0)
            {
                ...
                foreach (DictionaryEntry entry in (IEnumerable) properties)
                {
                    ...
                    writer.WriteAttributeString("name", Transform.MaskXmlInvalidCharacters((string) entry.Key, base.InvalidCharReplacement));
                    string str2 = null;
                    if (!this.Base64EncodeProperties)
                    {
                        str2 = Transform.MaskXmlInvalidCharacters(loggingEvent.Repository.RendererMap.FindAndRender(entry.Value), base.InvalidCharReplacement);
                    }
                    else
                    {
