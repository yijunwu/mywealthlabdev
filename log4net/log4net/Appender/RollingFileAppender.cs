    ...
    public class RollingFileAppender : FileAppender
    {
        ...
        protected void DeleteFile(string fileName)
        {
            if (this.FileExists(fileName))
            {
                ...
                {
                    LogLog.Debug("RollingFileAppender: Exception while moving file to be deleted [" + fileName + "] -> [" + destFileName + "]", exception);
                }
                try
                {
                    using (disposable = base.SecurityContext.Impersonate(this))
                ...
                catch (Exception exception2)
                {
                    ...
                    {
                        LogLog.Debug("RollingFileAppender: Exception while deleting temp file [" + path + "]", exception2);
                    }
                }
            }
        }
        ...
        private ArrayList GetExistingFiles(string baseFilePath)
        {
            ...
            using (base.SecurityContext.Impersonate(this))
            {
                ...
                if (Directory.Exists(path))
                {
                    ...
                    string[] files = Directory.GetFiles(path, GetWildcardPatternForFile(fileName));
                    if (files != null)
                    {
            ...
            if (curFileName.StartsWith(baseFile) && !curFileName.Equals(baseFile))
            {
                ...
                if (-1 != num)
                {
                    ...
                    else
                    {
                        ...
                        {
                            int num3;
                            if (SystemInfo.TryParse(curFileName.Substring(num + 1), out num3) && (num3 > this.m_curSizeRollBackups))
                            {
