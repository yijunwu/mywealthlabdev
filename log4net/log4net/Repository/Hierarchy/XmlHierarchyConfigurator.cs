    ...
    public class XmlHierarchyConfigurator
    {
            ...
            if ((element != null) && (this.m_hierarchy != null))
            {
                ...
                else
                {
                    if (!LogLog.InternalDebugging)
                    {
                        ...
                        LogLog.Debug("XmlHierarchyConfigurator: debug attribute [" + argValue + "].");
                        ...
                        string str3 = element.GetAttribute("configDebug");
                    ...
                    if ((attribute != null) && (attribute.Length > 0))
                    {
                        ...
                        {
                            LogLog.Error("XmlHierarchyConfigurator: Invalid update attribute value [" + attribute + "]");
                        }
                    }
                    LogLog.Debug("XmlHierarchyConfigurator: Configuration update mode [" + merge.ToString() + "].");
                    ...
                    string str5 = element.GetAttribute("threshold");
                    LogLog.Debug("XmlHierarchyConfigurator: Hierarchy Threshold [" + str5 + "]");
                    if ((str5.Length > 0) && (str5 != "null"))
                    {
                        Level level = (Level) this.ConvertStringTo(typeof(Level), str5);
                        if (level != null)
                        {
        ...
        protected object CreateObjectFromXml(XmlElement element, Type defaultTargetType, Type typeConstraint)
        {
            ...
            if ((typeConstraint != null) && !typeConstraint.IsAssignableFrom(c))
            {
                ...
                {
                    LogLog.Error("XmlHierarchyConfigurator: Object type [" + c.FullName + "] is not assignable to type [" + typeConstraint.FullName + "]. There are no acceptable type conversions.");
                    return null;
                }
                flag = true;
            }
            ...
            {
                LogLog.Error("XmlHierarchyConfigurator: Failed to construct object of type [" + c.FullName + "] Exception: " + exception2.ToString());
            }
            foreach (XmlNode node in element.ChildNodes)
            {
                if (node.NodeType == XmlNodeType.Element)
        ...
        protected IAppender ParseAppender(XmlElement appenderElement)
        {
            ...
            LogLog.Debug("XmlHierarchyConfigurator: Loading Appender [" + attribute + "] type: [" + typeName + "]");
            try
            {
                ...
                foreach (XmlNode node in appenderElement.ChildNodes)
                {
                    if (node.NodeType == XmlNodeType.Element)
                    {
                        ...
                        if (appenderRef.LocalName == "appender-ref")
                        {
                            ...
                            if (attachable != null)
                            {
                                LogLog.Debug("XmlHierarchyConfigurator: Attaching appender named [" + str3 + "] to appender named [" + target.Name + "].");
                            ...
                            {
                                LogLog.Error("XmlHierarchyConfigurator: Requesting attachment of appender named [" + str3 + "] to appender named [" + target.Name + "] which does not implement log4net.Core.IAppenderAttachable.");
                            }
                        }
                        else
                        {
            ...
            {
                LogLog.Error("XmlHierarchyConfigurator: Could not create Appender [" + attribute + "] of type [" + typeName + "]. Reported error follows.", exception);
                return null;
            }
        }
        ...
        protected void ParseLevel(XmlElement element, Logger log, bool isRoot)
        {
            ...
            LogLog.Debug("XmlHierarchyConfigurator: Logger [" + name + "] Level string is [" + attribute + "].");
            if ("inherited" == attribute)
            {
                ...
                {
                    LogLog.Debug("XmlHierarchyConfigurator: Logger [" + name + "] level set to inherit from parent.");
                    log.Level = null;
                }
            }
            else
            {
                ...
                if (log.Level == null)
                {
                    LogLog.Error("XmlHierarchyConfigurator: Undefined level [" + attribute + "] on Logger [" + name + "].");
                }
                else
                {
                    LogLog.Debug(string.Concat(new object[] { "XmlHierarchyConfigurator: Logger [", name, "] level set to [name=\"", log.Level.Name, "\",value=", log.Level.Value, "]." }));
                }
            }
        }
        ...
        protected void ParseLogger(XmlElement loggerElement)
        {
            ...
            lock (log)
            {
                ...
                LogLog.Debug(string.Concat(new object[] { "XmlHierarchyConfigurator: Setting [", log.Name, "] additivity to [", flag, "]." }));
                log.Additivity = flag;
        ...
        protected void ParseRenderer(XmlElement element)
        {
            ...
            LogLog.Debug("XmlHierarchyConfigurator: Rendering class [" + attribute + "], Rendered class [" + typeName + "].");
            ...
            else
            {
                ...
                {
                    LogLog.Error("XmlHierarchyConfigurator: Could not find class [" + typeName + "].", exception);
                }
            }
        }
        ...
        protected void SetParameter(XmlElement element, object target)
        {
            ...
            {
                LogLog.Error("XmlHierarchyConfigurator: Cannot find Property [" + attribute + "] to set object on [" + target.ToString() + "]");
            }
            else
            {
                ...
                if (innerText != null)
                {
                    ...
                    if ((typeName != null) && (typeName.Length > 0))
                    {
                        try
                        {
                            ...
                            LogLog.Debug("XmlHierarchyConfigurator: Parameter [" + attribute + "] specified subtype [" + c.FullName + "]");
                            if (!propertyType.IsAssignableFrom(c))
                            {
                                ...
                                {
                                    LogLog.Error("XmlHierarchyConfigurator: Subtype [" + c.FullName + "] set on [" + attribute + "] is not a subclass of property type [" + propertyType.FullName + "] and there are no acceptable type conversions.");
                                }
                            }
                            else
                            {
                        ...
                        {
                            LogLog.Error("XmlHierarchyConfigurator: Failed to find type [" + typeName + "] set on [" + attribute + "]", exception);
                        }
                    }
                    object sourceInstance = this.ConvertStringTo(propertyType, innerText);
                    if ((sourceInstance != null) && (type3 != null))
                    {
                        LogLog.Debug("XmlHierarchyConfigurator: Performing additional conversion of value from [" + sourceInstance.GetType().Name + "] to [" + type3.Name + "]");
                    ...
                    if (sourceInstance != null)
                    {
                        if (property != null)
                        {
                            LogLog.Debug("XmlHierarchyConfigurator: Setting Property [" + property.Name + "] to " + sourceInstance.GetType().Name + " value [" + sourceInstance.ToString() + "]");
                            try
                            {
                            ...
                            catch (TargetInvocationException exception4)
                            {
                                ...
                                LogLog.Error(string.Concat(new object[] { "XmlHierarchyConfigurator: Failed to set parameter [", property.Name, "] on object [", target, "] using value [", sourceInstance, "]" }), exception2.InnerException);
                            }
                        }
                        else if (info2 != null)
                        {
                            LogLog.Debug("XmlHierarchyConfigurator: Setting Collection Property [" + info2.Name + "] to " + sourceInstance.GetType().Name + " value [" + sourceInstance.ToString() + "]");
                            try
                            {
                            ...
                            catch (TargetInvocationException exception5)
                            {
                                ...
                                LogLog.Error(string.Concat(new object[] { "XmlHierarchyConfigurator: Failed to set parameter [", attribute, "] on object [", target, "] using value [", sourceInstance, "]" }), exception2.InnerException);
                            }
                        }
                    }
                    else
                    {
                        LogLog.Warn(string.Concat(new object[] { "XmlHierarchyConfigurator: Unable to set property [", attribute, "] on object [", target, "] using value [", innerText, "] (with acceptable conversion types)" }));
                    }
                }
                else
                {
                    ...
                    if (obj3 == null)
                    {
                        LogLog.Error("XmlHierarchyConfigurator: Failed to create object to set param: " + attribute);
                    }
                    else if (property != null)
                    {
                        LogLog.Debug(string.Concat(new object[] { "XmlHierarchyConfigurator: Setting Property [", property.Name, "] to object [", obj3, "]" }));
                        try
                        {
                        ...
                        catch (TargetInvocationException exception6)
                        {
                            ...
                            LogLog.Error(string.Concat(new object[] { "XmlHierarchyConfigurator: Failed to set parameter [", property.Name, "] on object [", target, "] using value [", obj3, "]" }), exception2.InnerException);
                        }
                    }
                    else if (info2 != null)
                    {
                        LogLog.Debug(string.Concat(new object[] { "XmlHierarchyConfigurator: Setting Collection Property [", info2.Name, "] to object [", obj3, "]" }));
                        try
                        {
                        ...
                        catch (TargetInvocationException exception7)
                        {
                            ...
                            LogLog.Error(string.Concat(new object[] { "XmlHierarchyConfigurator: Failed to set parameter [", info2.Name, "] on object [", target, "] using value [", obj3, "]" }), exception2.InnerException);
                        }
                    }
                }
            }
