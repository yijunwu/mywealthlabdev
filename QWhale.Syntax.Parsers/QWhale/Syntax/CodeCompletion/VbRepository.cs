namespace QWhale.Syntax.CodeCompletion
{
    ...
    public class VbRepository : ReflectionRepository
    {
        public VbRepository(bool caseSensitive, ISyntaxTree syntaxTree) : base(caseSensitive, syntaxTree)
        {
        }
        ...
        public override object GetSpecialMemberType(string text, ISyntaxNode node, ref string name, ref Point position, ref Point endPos, out CodeCompletionScope scope)
        {
            ...
            if (((obj2 == null) && NETRepository.IsDeclarationReference(node, false)) && text.Trim().ToLower().EndsWith(VbLexerToken.Overrides.ToString().ToLower()))
            {
